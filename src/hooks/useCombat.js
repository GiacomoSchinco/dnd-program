...
import { normalizeCombatRecord, sortParticipantsByInitiative, getNextMonsterName, normalizeCombatStatus } from './utils'

export function useCombat() {
  const activeCombat = useLiveQuery(async () => normalizeCombatRecord(await db.activeCombat.get('current')))
  const combats = useLiveQuery(async () => (await db.combats.orderBy('date').reverse().toArray()).map(normalizeCombatRecord), [], [])
  ...
  const persistLinkedCombat = useCallback(async (combatState) => {
    ...
    await db.combats.update(combatState.combatId, {
      name: combatState.name || 'Battaglia', date: new Date().toISOString(), status: normalizeCombatStatus(combatState.status),
      participants: combatState.participants ?? [], currentTurnIndex: combatState.currentTurnIndex ?? 0, round: combatState.round ?? 1,
  ...
  const applyDamage = useCallback(async (participantId, amount) => {
    try {
      ...
      const nextState = { ...current, participants: current.participants.map((p) => p.id === participantId ? { ...p, currentHp: newHp } : p) }
  ...
  const sortByInitiative = useCallback(async () => {
    ...
    const nextState = { ...current, participants: sorted, currentTurnIndex: nextTurnIndex }
    await db.activeCombat.put(nextState)
    await persistLinkedCombat(nextState)
  }, [persistLinkedCombat])
  ...
  const addParticipant = useCallback(async (participant) => {
    try {
      ...
      const participantWithName = participant?.type === 'monster' ? { ...participant, name: getNextMonsterName(participant?.name, currentParticipants) } : participant
      ...
      if (!current) {
        ...
        const nextState = { id: 'current', name: 'Nuovo Combattimento', status: 'prepared', participants: sortedParticipants, currentTurnIndex: 0, round: 1 }
        await db.activeCombat.put(nextState)
        await persistLinkedCombat(nextState)
      } else {
        const currentParticipantId = current.participants?.[current.currentTurnIndex]?.id ?? null
        ...
        const nextState = { ...current, participants: sortedParticipants, currentTurnIndex: nextTurnIndex }
        await db.activeCombat.put(nextState)
        await persistLinkedCombat(nextState)
      }
  ...
  const removeParticipant = useCallback(async (participantId) => {
    try {
      ...
      const nextState = { ...current, participants: filtered, currentTurnIndex: newIndex }
      await db.activeCombat.put(nextState)
      await persistLinkedCombat(nextState)
    } catch { toast.error('Errore nella rimozione del partecipante') }
  }, [persistLinkedCombat])

  const updateParticipantInitiative = useCallback(async (participantId, initiative) => {
    const current = await db.activeCombat.get('current')
    if (!current) return
    const currentParticipantId = current.participants?.[current.currentTurnIndex]?.id ?? null
    ...
    try {
      ...
      if (current.combatId) {
        await db.combats.update(current.combatId, {
          name: name || current.name || 'Combattimento', date: new Date().toISOString(), status: normalizeCombatStatus(current.status),
          participants: current.participants, currentTurnIndex: current.currentTurnIndex, round: current.round ?? 1, campaignId: current.campaignId ?? null,
        })
        return
      }
  ...
  const loadFromHistory = useCallback(async (combatId) => {
    try {
      ...
      await db.activeCombat.put({
        id: 'current', combatId: combat.id, campaignId: combat.campaignId ?? null, name: combat.name, status: normalizeCombatStatus(combat.status),
        participants: sortedParticipants, currentTurnIndex: 0, round: combat.round ?? 1,
      })
  ...
  const createCombatForCampaign = useCallback(async (campaignId, name) => {
    try {
      ...
      const combatRecord = { name: name?.trim() || 'Nuova Battaglia', date: new Date().toISOString(), status: 'prepared', campaignId, participants: [], currentTurnIndex: 0, round: 1 }
  ...
  const newCombat = useCallback(async () => {
    const current = await db.activeCombat.get('current')
    const nextState = { id: 'current', combatId: current?.combatId, campaignId: current?.campaignId ?? null, name: current?.name || 'Nuovo Combattimento', status: 'prepared', participants: [], currentTurnIndex: 0, round: 1 }
    await db.activeCombat.put(nextState)
    await persistLinkedCombat(nextState)
  }, [persistLinkedCombat])
  ...
  return {
    ...
    completeCombat: (id) => setCombatStatus(id, 'terminated'),
    terminateCombat: (id) => setCombatStatus(id, 'terminated'),
    deleteFromHistory,
    newCombat,
  }
}
